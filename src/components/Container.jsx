...
export default function Container({ data }) {
    ...
    const dataSet = use(data)
    const [dataList, setDataList] = useState(dataSet)
    const handleClick = (d) => {
        if (d === "All") setDataList(dataSet)
        else setDataList(dataSet.filter(e => e.status === d))
        setActive(d)
    }
    return (
        <main>
            ...
            <section id='hero' className='grid grid-cols-1 lg:grid-cols-3 min-h-[50vh] md:min-h-[70vh] items-center justify-items-center text-white w-11/12 mx-auto gap-4'>
                <div className='w-full relative h-5/6 lg:h-1/2 bg-linear-135 from-indigo-900 to-violet-700 rounded-xl flex flex-col items-center justify-center'>
                    ...
                    <h4 className='text-5xl font-medium'>{dataSet.filter(e => e.status === "Pending").length}</h4>
                </div>
                <div className='w-full relative h-5/6 lg:h-1/2 bg-linear-135 from-slate-500 to-gray-700 rounded-xl flex flex-col items-center justify-center'>
                    ...
                    <h4 className='text-5xl font-medium'>{dataSet.filter(e => e.status === "Submitted").length}</h4>
                </div>
                <div className='w-full relative h-5/6 lg:h-1/2 bg-linear-135 from-emerald-700 to-emerald-900 rounded-xl flex flex-col items-center justify-center'>
                    ...
                    <h4 className='text-5xl font-medium'>{dataSet.filter(e => e.status === "Reviewed").length}</h4>
                </div>
            </section>
            <div className="flex items-center justify-between gap-4 font-semibold w-11/12 mx-auto">
                ...
                <span className="bg-purple-200 rounded-sm border-2 border-purple-700">
                    <button onClick={() => handleClick("All")} className={`px-4 py-2 cursor-pointer ${active === "All" ? "bg-purple-950 text-white hover:bg-purple-900" : "hover:bg-purple-300"} border-r-2 border-purple-700`}>All</button>
                    <button onClick={() => handleClick("Pending")} className={`px-4 py-2 cursor-pointer ${active === "Pending" ? "bg-purple-950 text-white hover:bg-purple-900" : "hover:bg-purple-300"} border-r-2 border-purple-700`}>Pending</button>
                    <button onClick={() => handleClick("Submitted")} className={`px-4 py-2 cursor-pointer ${active === "Submitted" ? "bg-purple-950 text-white hover:bg-purple-900" : "hover:bg-purple-300"} border-r-2 border-purple-700`}>Submitted</button>
                    <button onClick={() => handleClick("Reviewed")} className={`px-4 py-2 cursor-pointer ${active === "Reviewed" ? "bg-purple-950 text-white hover:bg-purple-900" : "hover:bg-purple-300"}`}>Reviewed</button>
                </span>
            </div>
